...
//right now i just limit it to the first 20 responses from the db
...
if (iteration === 6*60) {
    ...
    for (let i=0; i<+dbUser.allowable_tracking_num; i++){
    newDb.push(dbResponse[i])
    }
    

    console.log('can they cheat?', dbResponse.length, newDb.length, 'NewDB length')
