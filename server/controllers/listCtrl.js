...
module.exports = {
  ...
  addList: async (req, res) => {
    ...
    properties.forEach(async (el) => {
      ...
      el.Seller, listName, latitude, longitude, 't'])
      .catch(err => {
    ...
    setTimeout(async () => {
      ...
      let trackedProps = await dbInstance.get_total_tracked([session.user.user_id])
  ...
  addIndividual: async (req, res) => {
    ...
    try {
      ...
      let totalTracked= await dbInstance.get_total_tracked([session.user.user_id])
      ...
      dbInstance.add_property([street, city, state, zip, price, bathrooms, bedrooms, seller, newListName, latitude, longitude, track])
